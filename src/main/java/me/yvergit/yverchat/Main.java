...
public class Main {
	...
	private static String secret = System.getenv("YVCHAT_SECRET");
	private static String tcSecret = System.getenv("YVCHAT_TRIPCODE_SECRET");
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		if (settingsJson.has("discord")) {
			...
			if (discordJson.has("enabled") && discordJson.get("enabled").getAsBoolean() && discordJson.has("token") && discordJson.has("channels")) {
				String token = System.getenv("YVCHAT_TOKEN");
		...
		if (settingsJson.has("chat_filter")) {
			...
			if (chatFilterJson.has("enabled") && chatFilterJson.get("enabled").getAsBoolean() && chatFilterJson.has("key")) {
				chatFilterKey = System.getenv("YVCHAT_CHAT_FILTER_KEY");
		...
		System.out.println("Starting yverchat by yvergit on " + host + ":" + port + "!");
					...
					if (filter) {
						for (int ii = 0; ii < 2; ii++) {
							try (AsyncHttpClient asyncHttpClient = Dsl.asyncHttpClient()) {
								...
								jo.addProperty("communityId", "yvchat");
